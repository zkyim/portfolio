...
const SkillsSection = () => {
  const [forntend, setForntend] = useState([]);
  const [backend, setBackend] = useState([]);
  const [tools, setTools] = useState([]);
  useEffect(()=> {
    skillApis.getFrontendSkills().then(res => {
      setForntend(res.data.data);
    })
    skillApis.getBackendSkills().then(res => {
      setBackend(res.data.data);
    })
    skillApis.getToolsSkills().then(res => {
